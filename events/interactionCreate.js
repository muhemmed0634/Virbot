// ==========================================
//  VirBot — interactionCreate Olayı
//  Ticket, Doğrulama, Çekiliş Katılım
// ==========================================
'use strict';

const { ticketAc, ticketKapat } = require('../modules/ticket/ticketManager');
const { cekiliseKatil } = require('../modules/giveaway/giveawayManager');
  ...
  async calistir(client, interaction) {
    if (interaction.isButton()) {
      ...
      // ── DOĞRULAMA SİSTEMİ ──
        ...
        if (!rol) {
          return interaction.reply({ content: '❌ Doğrulama rolü bulunamadı, sunucu yetkililerine haber verin.', ephemeral: true });
        }
        ...
        await interaction.reply({ content: `✅ Başarıyla doğrulandınız ve **${rol.name}** rolünü aldınız!`, ephemeral: true });
      }

      // ── ÇEKİLİŞ SİSTEMİ ──
      else if (customId.startsWith('cekilis_katil_')) {
        const cekilisMesajId = customId.split('_')[2];
      ...
      // ── GEÇİCİ SES KONTROLLERİ ──
        ...
        if (!kanal) {
          return interaction.reply({ content: '❌ Bu butonları kullanmak için bir ses kanalında olmalısınız.', ephemeral: true });
        }
        ...
        if (sahipId !== interaction.user.id && !yetkiliMi(interaction.member)) {
          return interaction.reply({ content: '❌ Bu kanalın sahibi siz değilsiniz!', ephemeral: true });
        }
